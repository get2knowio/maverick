...
class TestBuiltinWorkflowInfo:
    ...
    def test_all_workflow_info_constants_exist(self) -> None:
        ...
        assert FLY_WORKFLOW_INFO is not None
        assert REFUEL_WORKFLOW_INFO is not None
    ...
    def test_workflow_info_fields_are_correct(self) -> None:
        """Test that FLY_WORKFLOW_INFO has correct fields."""
        assert FLY_WORKFLOW_INFO.name == "fly"
        assert FLY_WORKFLOW_INFO.description == "Full spec-based development workflow"
        assert isinstance(FLY_WORKFLOW_INFO.inputs, tuple)
        assert len(FLY_WORKFLOW_INFO.inputs) == 3
        assert FLY_WORKFLOW_INFO.step_summary.startswith("init →")
    ...
    def test_workflow_info_inputs_structure(self) -> None:
        ...
        first_input = FLY_WORKFLOW_INFO.inputs[0]
...
class TestListWorkflows:
    ...
    def test_workflow_info_objects_match_constants(self, library: BuiltinLibrary) -> None:
        ...
        assert workflow_dict["fly"] == FLY_WORKFLOW_INFO
...
class TestHasFragment:
    ...
    def test_returns_false_for_workflow_names(self, library: BuiltinLibrary) -> None:
        ...
        assert library.has_fragment("fly") is False
        assert library.has_fragment("refuel") is False
...
class TestBuiltinLibraryIntegration:
    ...
    def test_workflow_info_matches_loaded_workflow(self, library: BuiltinLibrary) -> None:
        ...
        workflows = library.list_workflows()
        fly_info = next(wf for wf in workflows if wf.name == "fly")

        # Load actual workflow
        fly_workflow = library.get_workflow("fly")
