...
class TestSpecificWorkflows:
    ...
    def test_fly_workflow_structure(self) -> None:
        """Test that fly workflow has expected structure."""
        # Arrange
        workflow_path = WORKFLOWS_DIR / "fly.yaml"
        yaml_content = workflow_path.read_text()
        workflow = parse_workflow(yaml_content, validate_only=True)
    ...
    def test_refuel_workflow_structure(self) -> None:
        """Test that refuel workflow has expected structure."""
        # Arrange
        workflow_path = WORKFLOWS_DIR / "refuel.yaml"
        yaml_content = workflow_path.read_text()
        workflow = parse_workflow(yaml_content, validate_only=True)

        # Assert
        assert workflow.name == "refuel"
