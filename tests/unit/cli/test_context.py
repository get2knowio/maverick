...
# =============================================================================
# FlyCommandInputs Tests
# =============================================================================


class TestFlyCommandInputs:
    """Tests for FlyCommandInputs dataclass."""

    def test_fly_command_inputs_required_field(self) -> None:
        """Test FlyCommandInputs requires branch_name."""
        inputs = FlyCommandInputs(branch_name="feature/my-feature")

        assert inputs.branch_name == "feature/my-feature"

    def test_fly_command_inputs_default_values(self) -> None:
        """Test FlyCommandInputs default values."""
        inputs = FlyCommandInputs(branch_name="main")

        assert inputs.branch_name == "main"
    ...
    def test_fly_command_inputs_with_all_fields(self) -> None:
        """Test FlyCommandInputs with all fields set."""
        task_file = Path("/path/to/tasks.md")
        inputs = FlyCommandInputs(
        ...
        assert inputs.branch_name == "feature/test"
        assert inputs.task_file == task_file
        assert inputs.skip_review is True
        assert inputs.skip_pr is True
        assert inputs.dry_run is True

    def test_fly_command_inputs_is_frozen(self) -> None:
        """Test FlyCommandInputs dataclass is frozen (immutable)."""
        inputs = FlyCommandInputs(branch_name="test")

        with pytest.raises(AttributeError):
            inputs.branch_name = "new-branch"  # type: ignore[misc]

        with pytest.raises(AttributeError):
            inputs.skip_review = True  # type: ignore[misc]

    def test_fly_command_inputs_with_path_object(self) -> None:
        """Test FlyCommandInputs accepts Path object for task_file."""
        path = Path("/project/tasks.md")
        inputs = FlyCommandInputs(branch_name="main", task_file=path)

        assert inputs.task_file == path
        assert isinstance(inputs.task_file, Path)

    def test_fly_command_inputs_with_skip_flags(self) -> None:
        """Test FlyCommandInputs with skip flags."""
        inputs_review = FlyCommandInputs(branch_name="test", skip_review=True)
        inputs_pr = FlyCommandInputs(branch_name="test", skip_pr=True)

        assert inputs_review.skip_review is True
        assert inputs_review.skip_pr is False

        assert inputs_pr.skip_pr is True
        assert inputs_pr.skip_review is False

    def test_fly_command_inputs_has_slots(self) -> None:
        """Test FlyCommandInputs uses slots for memory efficiency."""
        inputs = FlyCommandInputs(branch_name="test")
...
# =============================================================================
# RefuelCommandInputs Tests
# =============================================================================


class TestRefuelCommandInputs:
    """Tests for RefuelCommandInputs dataclass."""

    def test_refuel_command_inputs_default_values(self) -> None:
        """Test RefuelCommandInputs default values."""
        inputs = RefuelCommandInputs()
    ...
    def test_refuel_command_inputs_with_custom_label(self) -> None:
        """Test RefuelCommandInputs with custom label."""
        inputs = RefuelCommandInputs(label="bug")

        assert inputs.label == "bug"
        assert inputs.limit == 5
        assert inputs.parallel is True

    def test_refuel_command_inputs_with_custom_limit(self) -> None:
        """Test RefuelCommandInputs with custom limit."""
        inputs = RefuelCommandInputs(limit=10)

        assert inputs.label == "tech-debt"
        assert inputs.limit == 10
        assert inputs.parallel is True

    def test_refuel_command_inputs_with_parallel_disabled(self) -> None:
        """Test RefuelCommandInputs with parallel mode disabled."""
        inputs = RefuelCommandInputs(parallel=False)

        assert inputs.parallel is False
        assert inputs.label == "tech-debt"

    def test_refuel_command_inputs_with_dry_run(self) -> None:
        """Test RefuelCommandInputs with dry_run enabled."""
        inputs = RefuelCommandInputs(dry_run=True)

        assert inputs.dry_run is True
        assert inputs.label == "tech-debt"

    def test_refuel_command_inputs_with_all_fields(self) -> None:
        """Test RefuelCommandInputs with all fields set."""
        inputs = RefuelCommandInputs(
            label="enhancement",
            limit=3,
            parallel=False,
        ...
        assert inputs.label == "enhancement"
        assert inputs.limit == 3
        assert inputs.parallel is False
        assert inputs.dry_run is True

    def test_refuel_command_inputs_is_frozen(self) -> None:
        """Test RefuelCommandInputs dataclass is frozen (immutable)."""
        inputs = RefuelCommandInputs()

        with pytest.raises(AttributeError):
            inputs.label = "new-label"  # type: ignore[misc]

        with pytest.raises(AttributeError):
            inputs.limit = 10  # type: ignore[misc]

    def test_refuel_command_inputs_limit_zero(self) -> None:
        """Test RefuelCommandInputs with limit=0."""
        inputs = RefuelCommandInputs(limit=0)

        assert inputs.limit == 0

    def test_refuel_command_inputs_has_slots(self) -> None:
        """Test RefuelCommandInputs uses slots for memory efficiency."""
        inputs = RefuelCommandInputs()
...
class TestIntegration:
    ...
    def test_cli_context_with_command_inputs(
        ...
        fly_inputs = FlyCommandInputs(branch_name="feature/test")

        assert ctx.use_tui is True
        assert ctx.verbosity == 1
        assert fly_inputs.branch_name == "feature/test"
    ...
    def test_dataclasses_are_independent(self) -> None:
        """Test that command input dataclasses are independent."""
        fly = FlyCommandInputs(branch_name="test")
        refuel = RefuelCommandInputs(label="bug")
        review = ReviewCommandInputs(pr_number=1)

        # Should not interfere with each other
        assert fly.branch_name == "test"
        assert refuel.label == "bug"
