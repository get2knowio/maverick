...
class TestImplementerAgentInitialization:
    ...
    def test_default_initialization(self, agent: ImplementerAgent) -> None:
        ...
        # Instructions are rendered with skill guidance, so check key content
        assert "expert software engineer" in agent.instructions.lower()
        assert "test-driven" in agent.instructions.lower()
    ...
    def test_instructions_contains_tdd_approach(self, agent: ImplementerAgent) -> None:
        """Test instructions includes TDD methodology."""
        prompt = agent.instructions
        assert "TDD" in prompt or "test-driven" in prompt.lower()
        assert "test" in prompt.lower()

    def test_instructions_does_not_request_commit_message(self, agent: ImplementerAgent) -> None:
        """Test instructions output template does not include commit_message.

        Commit messages are generated by the workflow's CommitMessageGenerator,
        not by the implementer agent. Including commit_message in the output
        template wastes tokens on a value that gets discarded.
        """
        prompt = agent.instructions
        # The output JSON template should not contain commit_message
        assert '"commit_message"' not in prompt

    def test_instructions_requires_tests(self, agent: ImplementerAgent) -> None:
        """Test instructions makes test creation mandatory, not optional."""
        prompt = agent.instructions
    ...
    def test_instructions_contains_framework_conventions(self, agent: ImplementerAgent) -> None:
        """Test instructions includes framework conventions for runtime guidance.
    ...
    def test_instructions_is_bead_aware(self, agent: ImplementerAgent) -> None:
        """Test instructions uses bead-driven framing."""
        prompt = agent.instructions
        assert "bead" in prompt.lower()

    def test_allowed_tools_includes_required_tools(self, agent: ImplementerAgent) -> None:
    ...
    def test_allowed_tools_uses_centralized_constants(self, agent: ImplementerAgent) -> None:
...
class _RemovedTestExecuteParallelBatch:  # noqa: F841
    ...
    async def test_execute_parallel_batch_runs_concurrently(
        ...
        with patch.object(agent, "_execute_single_task", new_callable=AsyncMock) as mock_execute:
    ...
    async def test_execute_parallel_batch_returns_results_in_order(
        ...
        tasks = [Task(id=f"T{i:03d}", description=f"Test {i}", parallel=True) for i in range(1, 6)]
        context = ImplementerContext(
            task_description="Parallel task batch execution test",
            branch="test",
            cwd=tmp_path,
...
class _RemovedTestErrorHandling:  # noqa: F841
    ...
    @pytest.mark.asyncio
    async def test_handles_task_parse_error(self, agent: ImplementerAgent, tmp_path: Path) -> None:
...
""")
    ...
    @pytest.mark.asyncio
    async def test_handles_validation_skip(self, agent: ImplementerAgent, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handles_dry_run(self, agent: ImplementerAgent, tmp_path: Path) -> None:
...
class TestSideEffectFree:
    ...
    @pytest.mark.asyncio
    async def test_agent_has_no_validation_method(self, agent: ImplementerAgent) -> None:
    ...
    @pytest.mark.asyncio
    async def test_agent_has_no_create_commit_method(self, agent: ImplementerAgent) -> None:
