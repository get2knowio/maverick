...
def test_get_builtin_path_contains_expected_workflows() -> None:
    ...
    fly_workflow = builtin_path / "workflows" / "fly.yaml"
    assert fly_workflow.exists()
...
def test_discover_get_workflow_lookup() -> None:
    ...
    assert fly_workflow.workflow.name == "fly"
    assert fly_workflow.source == WorkflowSource.BUILTIN.value
...
def test_discover_project_overrides_builtin(
    ...
    project_workflow = project_dir / "fly.yaml"
    # Modify the YAML to use "fly" name
    custom_yaml = sample_workflow_yaml.replace("test-workflow", "fly")
    ...
    assert fly.source == WorkflowSource.PROJECT.value

    # File path should be the project file
    assert fly.file_path == project_workflow

    # Should have overrides tuple containing the builtin path
    assert len(fly.overrides) > 0
    # Check that one of the overridden paths is from builtin
    builtin_path = discovery.get_builtin_path()
    overridden_paths_str = [str(p) for p in fly.overrides]
...
def test_discovered_workflow_has_correct_source_builtin() -> None:
    ...
    fly = result.get_workflow("fly")
    assert fly is not None
    assert fly.source == WorkflowSource.BUILTIN.value
...
def test_discovered_workflow_structure() -> None:
    ...
    assert hasattr(fly, "workflow")
    assert hasattr(fly, "file_path")
    assert hasattr(fly, "source")
    assert hasattr(fly, "overrides")

    # Check types
    assert isinstance(fly.file_path, Path)
    assert isinstance(fly.source, str)
    assert isinstance(fly.overrides, tuple)

    # File path should exist
    assert fly.file_path.exists()

    # Workflow should have name and description
    assert fly.workflow.name == "fly"
    assert fly.workflow.description
