...
def component_reg() -> ComponentRegistry:
    ...
    registry.generators.register("code_reviewer", MockAgent)  # type: ignore
...
def test_resolve_references_context_builder(component_reg: ComponentRegistry) -> None:
    ...
    component_reg.context_builders.register("my_context", build_context)
...
def test_parse_workflow_reference_error(component_reg: ComponentRegistry) -> None:
    """Test parse_workflow with unresolved reference."""
    yaml_content = """
version: "1.0"
name: test-workflow
steps:
...
steps:
  - name: step1
    ...
    action: unknown_action
"""
    # In validate_only mode, should not raise for unknown references
    workflow = parse_workflow(yaml_content, component_reg, validate_only=True)
    assert isinstance(workflow, WorkflowFile)
...
steps:
  - name: step1
    ...
    action: unknown_action
"""
    # In lenient mode, should not raise for unknown references
    workflow = parse_workflow(yaml_content, lenient_registry)
    assert isinstance(workflow, WorkflowFile)
