...
def registry_with_agent():
    ...
    reg.agents.register("test_agent", MockAgent)
    return reg
...
def registry_with_async_agent():
    ...
    reg.agents.register("async_agent", MockAsyncAgent)
    return reg
...
def registry_with_generator():
    ...
    reg.generators.register("test_generator", MockGenerator)
    return reg
...
def registry_with_async_generator():
    ...
    reg.generators.register("async_generator", MockAsyncGenerator)
    return reg
...
def registry_with_async_context_builder():
    ...
    async def build_context_async(base_context):
...
class TestAgentStepExecution:
    ...
    async def test_agent_step_with_context_builder(
        ...
        registry.agents.register("inspector_agent", ContextInspectorAgent)
    ...
    async def test_agent_step_missing_agent(self, registry):
        ...
        assert "nonexistent_agent" in result.step_results[0].error
    ...
    async def test_agent_step_missing_context_builder(self, registry_with_agent):
        ...
        result = executor.get_result()
        assert result.success is False
        assert "nonexistent_builder" in result.step_results[0].error
    ...
    async def test_agent_step_agent_raises_exception(self, registry):
        ...
        registry.agents.register("failing_agent", FailingAgent)
...
class TestGenerateStepExecution:
    ...
    async def test_generate_step_with_context_builder(
        ...
        registry.generators.register("inspector_generator", ContextInspectorGenerator)
    ...
    async def test_generate_step_missing_generator(self, registry):
        ...
        result = executor.get_result()
        assert result.success is False
        assert "nonexistent_generator" in result.step_results[0].error
    ...
    async def test_generate_step_generator_raises_exception(self, registry):
        ...
        registry.generators.register("failing_generator", FailingGenerator)
...
class TestValidateStepExecution:
    ...
    async def test_validate_step_success(self, registry):
        ...
        result = executor.get_result()
        assert result.success is True
        # Validate step returns a dict with success and stages
    ...
    async def test_validate_step_multiple_stages(self, registry):
        ...
        stage_names = [s["stage"] for s in result.final_output["stages"]]
        assert stage_names == ["format", "lint", "typecheck", "test"]
...
class TestStepExecutionEdgeCases:
    ...
    async def test_step_with_invalid_condition_expression(self, registry):
        ...
        result = executor.get_result()
        # Step should be skipped due to condition evaluation error
        assert result.success is True
