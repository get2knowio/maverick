...
class TestValidateWorkflowIntegration:
    ...
    async def test_validate_workflow_with_fix_disabled(
        ...
        assert events[0].workflow_name == "validate"
    ...
    async def test_validate_workflow_with_fix_enabled_and_failure(
        ...
        Note: This test requires the validate-and-fix sub-workflow to be registered,
        which we'll mock by registering a minimal workflow.
        """
        with open(workflow_path) as f:
            workflow = parse_workflow(f.read())

        # Create a mock validate-and-fix workflow
...
""")
        ...
        # Register the mock sub-workflow
