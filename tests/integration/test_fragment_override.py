...
""")
    ...
    with patch.object(DefaultWorkflowDiscovery, "get_user_path", return_value=user_dir / "maverick" / "workflows"), \
         patch.object(DefaultWorkflowDiscovery, "get_project_path", return_value=project_dir / ".maverick" / "workflows"):
...
def test_fragment_user_overrides_builtin(discovery_setup: tuple[Path, Path], tmp_path: Path):
...
""")
    ...
    with patch.object(DefaultWorkflowDiscovery, "get_user_path", return_value=user_dir / "maverick" / "workflows"), \
         patch.object(DefaultWorkflowDiscovery, "get_builtin_path", return_value=builtin_root):
