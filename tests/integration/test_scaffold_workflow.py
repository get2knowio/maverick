...
class TestParallelTemplateScaffolding:
    ...
    def test_parallel_template_has_multiple_substeps(self, tmp_path: Path) -> None:
        """Parallel template should demonstrate multiple parallel substeps."""
        scaffolder = get_scaffolder()
        ...
        if parallel_steps:
            # At least one parallel step should have 2+ substeps
            assert any(len(s.steps) >= 2 for s in parallel_steps)
