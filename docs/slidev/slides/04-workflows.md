...
<div v-click class="mt-8 p-4 rounded-lg bg-green-500/20 border border-green-500 text-green-300 text-center">
  <strong>Result:</strong> 40-60% token reduction vs monolithic approach
</div>
...
<div v-click class="mt-6 text-sm opacity-70">
  Usage: <code>maverick workflow run fly -i branch_name=025-feature-branch</code>
</div>
...
# RefuelWorkflow - Tech Debt Resolution
...
<div v-click class="mt-6 text-sm opacity-70">
  Usage: <code>maverick workflow run refuel -i label=tech-debt -i limit=3</code>
</div>

<!--
RefuelWorkflow is designed for batch processing of technical debt and small fixes. It's the workflow you use when you want to knock out multiple GitHub issues in one go.
...
Selection phase: Analyzes the discovered issues to select up to N issues (default 3) that are non-conflicting and can be worked on safely in parallel.
...
This workflow is particularly powerful for sprint planning - you can label 20 tech debt issues, run refuel with limit=5, and have 5 PRs ready for review in one workflow execution.
-->
...
<div class="flex flex-col gap-2 mt-3">
  ...
  <div v-click class="px-3 py-2 rounded bg-slate-700/50 text-sm">
    <strong class="text-purple-300">.on_error</strong> - Fallback handlers
  </div>
  <div v-click class="px-3 py-2 rounded bg-slate-700/50 text-sm">
