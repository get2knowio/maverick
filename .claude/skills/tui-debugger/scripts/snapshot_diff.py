...
def print_diff(before: list[str], after: list[str], context: int = 3):
    ...
    for i, region in enumerate(regions, 1):
        ...
        if region['type'] == 'replace':
            ...
            for idx in range(max_lines):
                before_line = region['before_lines'][idx] if idx < len(region['before_lines']) else ''
                after_line = region['after_lines'][idx] if idx < len(region['after_lines']) else ''
    ...
    print("SUMMARY")
    print(f"{'='*60}")
    total_removed = sum(len(r['before_lines']) for r in regions if r['type'] in ('delete', 'replace'))
    total_added = sum(len(r['after_lines']) for r in regions if r['type'] in ('insert', 'replace'))
