...
async def main() -> None:
    ...
    if not temporal_host or not temporal_host.strip():
        logger.error("temporal_config_invalid", error="TEMPORAL_HOST cannot be empty", provided_value=temporal_host)
        sys.exit(1)
    ...
    try:
        client = await asyncio.wait_for(Client.connect(temporal_host), timeout=connection_timeout)
    ...
    try:
        ...
        done, pending = await asyncio.wait([worker_task, shutdown_task], return_when=asyncio.FIRST_COMPLETED)
        ...
        logger.error("worker_error", phase="execution", error_type=type(e).__name__, error_message=str(e))
        raise
        ...
        logger.info("worker_stopped", status="cleanup_complete")
...
def run_worker() -> None:
    ...
    try:
        ...
        logger.error("worker_failed", error_type=type(e).__name__, error_message=str(e))
        raise
