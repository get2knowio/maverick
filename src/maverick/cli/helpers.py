...
def detect_task_file(branch_name: str | None = None) -> Path | None:
    ...
    1. .specify/<branch_name>/tasks.md (if branch_name provided)
    ...
    if branch_name:
        potential_paths.append(Path(f".specify/{branch_name}/tasks.md"))
...
def get_workflow_history(count: int = 5) -> list[dict[str, str]]:
    """Get recent workflow history.

    Args:
        count: Maximum number of recent workflows to retrieve.

    Returns:
        List of workflow history entries with time_ago calculated.
    """
    ...
    try:
        ...
        recent_entries = history_store.get_recent(count=count)
...
def format_status_json(
    branch: str,
    task_file_found: bool,
    pending_tasks: int,
    completed_tasks: int,
    recent_workflows: list[dict[str, str]],
) -> str:
    """Format status information as JSON.

    Args:
        branch: Current git branch name.
        task_file_found: Whether a task file was found.
        pending_tasks: Number of pending tasks.
        completed_tasks: Number of completed tasks.
        recent_workflows: List of recent workflow history entries.
...
def format_status_text(
    ...
    if task_file_found:
        task_summary = f"Tasks: {pending_tasks} pending, {completed_tasks} completed"
