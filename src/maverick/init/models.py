...
class InitConfig(BaseModel):
    ...
    def to_yaml(self) -> str:
        ...
        Empty ``actors`` is dropped so the zero-config experience stays
        clean. ``agents`` is always emitted so users discover the
        per-role binding configuration surface. A short comment
        explaining the block is injected above it.
