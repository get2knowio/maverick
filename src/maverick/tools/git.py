...
# Module-level working directory (can be overridden for testing)
_cwd: Path | None = None
...
async def git_commit(args: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns:
        ...
        Error: {"isError": true, "error_code": "NOTHING_TO_COMMIT"|"INVALID_INPUT"|...}
    """
    # Extract arguments
    ...
    formatted_message = _format_commit_message(message, commit_type, scope, breaking)
    ...
    try:
        ...
        if return_code != 0:
            ...
            return _error_response(f"Git commit failed: {error_message}", "GIT_ERROR")
        ...
        if sha_return_code != 0:
            ...
            logger.warning("Commit created but couldn't retrieve SHA: %s", sha_stderr)
            commit_sha = "unknown"
        else:
...
async def git_push(args: dict[str, Any]) -> dict[str, Any]:
    ...
    # Execute push
    try:
        stdout, stderr, returncode = await _run_git_command(*push_args, cwd=working_dir)
...
async def git_create_branch(args: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        args: Tool arguments containing:
            ...
            - base (str, optional): Base branch to create from (default: current branch).
    ...
    # Validate branch name (no spaces, basic validation)
    if " " in branch_name or any(
...
def create_git_tools_server(
    ...
    tools registered. By default, it verifies prerequisites (git installed,
    in git repo) before creating the server.
    ...
    Raises:
        GitToolsError: If prerequisites not met (unless skip_verification=True).
    ...
    # Set module-level working directory
    _cwd = cwd
    ...
    if not skip_verification:
        try:
            ...
            msg = (
                "create_git_tools_server() cannot be called from async context. "
                "Use skip_verification=True or call from synchronous code."
            )
            raise GitToolsError(msg, check_failed="async_context")
        except RuntimeError:
            # No running event loop - safe to create a new one with asyncio.run()
            asyncio.run(_verify_git_prerequisites(cwd))
