...
Run `src/scripts/sync-branch.sh $ARGUMENTS` and parse the JSON output.
...
- **Send notification:** Run `src/scripts/notify.sh error "Merge conflicts detected in: $branch"`
...
**If status is "error":**
- **Send notification:** Run `src/scripts/notify.sh error "Setup error: [error message]"`
- Report the error (missing spec directory or tasks file)
- Halt execution

**If status is "ok":**
- Store `branch`, `spec_dir`, and `tasks_file` for use in subsequent steps
- **Send notification:** Run `src/scripts/notify.sh spec_start "Starting spec: $branch"`
- Proceed to Part 1
...
First, run `src/scripts/get-changed-files.sh` to identify changed files.
...
**Send notification:** Run `src/scripts/notify.sh testing "Entering testing phase for: $branch"`

Run `src/scripts/run-validation.sh` and parse results.

**If `all_passed` is true:** Proceed to Part 3

**If any check failed:**
1. **Send notification:** Run `src/scripts/notify.sh error "Validation failed for: $branch"`
...
3. **Run PR script:**
   ```bash
   src/scripts/manage-pr.sh "feat(scope): description" /tmp/pr-body.md
   ```

4. **Report the PR URL to the user**

5. **Send notification:** Run `src/scripts/notify.sh complete "Spec complete: $branch - PR created"`
