...
- **FR-001**: System MUST provide a `build_implementation_context(task_file: Path, git: GitOperations) -> dict` function that returns a dict with `tasks`, `conventions`, `branch`, and `recent_commits` keys
...
- **FR-007**: System MUST provide a `fit_to_budget(sections: dict[str, str], budget: int) -> dict[str, str]` utility that proportionally truncates sections to fit token budget
...
- **TruncationMetadata**: Information about what was truncated, including original size, truncated size, and which sections were affected
- **ValidationOutput**: External type from validation module containing error details (file paths, line numbers, error messages)
- **GitHubIssue**: External type representing a GitHub issue with title, body, labels, and comments
- **GitOperations**: External type providing git functionality (branch info, diffs, file reading)
