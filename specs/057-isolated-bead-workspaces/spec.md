...
- **Hard interruption during fold-back or undo**: the process is killed
  ...
  rather than proceeding on top of it. Detection must not depend on inspecting
  the checkout's contents — the application is recorded durably before it
  begins and the record cleared only once it finishes, so a later run finds
  the record rather than inferring from what it sees.
...
- **Two runs in one checkout**: a second isolated run starts while another is
  still live in the same checkout. The second must refuse to start rather than
  interleave — it must not sweep or reuse the first's live workspaces, and must
  never fold back over a verification window belonging to the first.
...
- **FR-002**: The primitive MUST provision isolation per unit of work, keyed
  ...
  isolated copy.
- **FR-003**: The isolated copy MUST present the checkout's current source
  tree — including uncommitted work present when the unit starts — so the
  agent sees what it would see on the normal path.
...
- **FR-048**: Only one isolated run may be live in a given checkout at a time.
  A second MUST refuse to start with an actionable error identifying the
  holder, MUST NOT sweep or reuse the live run's workspaces, and MUST NOT fold
  back while another run's verification window is open.
- **FR-049**: Before applying a fold-back or an undo, the system MUST durably
  record that the application is in progress, and MUST clear that record only
  once it completes. A subsequent run that finds an uncleared record MUST
  refuse to proceed and MUST report which unit was mid-application, which
  operation it was, and the steps required to recover — it MUST NOT infer the
  situation from the checkout's contents, and MUST NOT roll back automatically.
- **FR-050**: Provisioning, fold-back, and teardown combined MUST add no more
  than 5 seconds of overhead per unit of work.
- **FR-051**: Every isolation lifecycle transition — provision, agent-step
  boundary, artifact-level checks, fold-back, environment-level checks, undo,
  teardown, and sweep — MUST be logged with the unit's identity and its
  workspace, and fold-backs, undos, verification rejections, and conflicts MUST
  additionally appear on the run's user-visible progress output.
...
- **In-Progress Application Record**: the durable marker written before a
  fold-back or undo and cleared on completion. Identifies the unit and the
  operation, and is what a later run reads to detect an interrupted
  application instead of guessing from the checkout.
...
- **SC-012**: Isolation adds no more than 5 seconds of overhead per unit of
  work, measured as the difference in wall-clock time between an isolated and
  a normal run of the same units, divided by the number of units.
- **SC-013**: After any interruption, concurrent-run attempt, or undo, an
  operator can reconstruct what happened to the checkout from the run's log
  and progress output alone, without inspecting version-control internals.
...
- **Q**: Does an agent working in isolation see uncommitted changes sitting in
  the user's checkout, or only committed state? → **A**: The isolated copy
  reflects the checkout's current tree, uncommitted work included (FR-003).
- **Q**: What happens when a second isolated run starts in a checkout where
  one is already in flight? → **A**: Refuse the second run immediately with a
  clear error (FR-048).
- **Q**: How does the next run discover that a previous one was killed partway
  through folding a delta in or undoing one? → **A**: A durable record written
  before the application and cleared after; a stale record refuses the next
  run with recovery steps, and never triggers an automatic rollback (FR-049).
- **Q**: What overhead may isolation add per unit of work? → **A**: At most 5
  seconds per unit for provision, fold-back, and teardown combined (FR-050,
  SC-012).
- **Q**: What must be observable about isolation while a run is happening? →
  **A**: Every lifecycle transition logged with unit identity and workspace;
  fold-backs, undos, rejections, and conflicts also on the run's progress
  output (FR-051, SC-013).
